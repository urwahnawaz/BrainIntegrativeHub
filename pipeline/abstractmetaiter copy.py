    ...
    def _writeHDF5Columns(self, fileName, hdf5Group, noneType="NA"):
        ...
        for i in range(1, len(lines[0])):
            for k in range(len(allTypes)):
                try:
                    #Attempt to parse all as this type, if there is no exception it is likely correct
                    colType = allTypes[k]
                    for j in range(len(lines)):
                        currValue = colType(lines[j][i])
                        if(currValue == noneType):
                            currValue = allDefaults[k]
                        values[j] = currValue
                    ...
                    if(allTypes[k] == str):
                        ...
                        for m in range(len(values)):
                            currValue = allDefaults[k] if values[m] == noneType else values[m]
                            if currValue not in valueDic:
                                valueDic[currValue] = valueId
                                valueId += 1
                        
                        if valueId >= 100:
                            #Too many unique values for plotting, try again
                            continue

                        #Save strings as indices to the dataset's symbols attribute
                        arr = np.array([valueDic[x] for x in values], dtype=allTypesNp[k])
                        dataset = hdf5Group.create_dataset(heading[i], data=arr, chunks=arr.shape, compression="gzip", compression_opts=9)
                        dataset.attrs.create("symbols", sorted(valueDic, key=valueDic.get))
                    else:
                        # Write float/int dataset
                        arr = np.array(values, dtype=allTypesNp[k])
                    ...
                    #Casting to type failed, try again
                    continue
            else:
                # Completely empty column, remove
                heading.pop(i)
